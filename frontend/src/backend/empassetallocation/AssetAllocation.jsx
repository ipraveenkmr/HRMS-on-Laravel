...
export default function EmpAsset() {
  ...
  const myassets = usecdotStore((state) => state.myassets).sort((a, b) =>
    a.id > b.id ? -1 : 1
  );
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - myassets.length) : 0;
  ...
  return (
    <>
      <Typography align="center" variant="h5">Assigned Assets</Typography>
      <Modal
        open={open}
        onClose={handleClose}
              ...
              <TableRow key={row.id}>
                <TableCell style={{ width: 160 }}>
                  {assetlist.map((item, index) => {
                    return <>{item.id === row.asset && item.asset_name}</>;
                  })}
                </TableCell>
                <TableCell style={{ width: 160 }}>
                  {employees.map((item, index) => {
                    return <>{item.id === row.employee && item.emp_name}</>;
                  })}
                </TableCell>
                <TableCell style={{ width: 160 }}>
                  {moment(row.allocation_date).format("DD-MM-YYYY")}
                </TableCell>
                <TableCell style={{ width: 160 }}>
                  {moment(row.return_date).format("DD-MM-YYYY")}
                </TableCell>
                <TableCell style={{ width: 160 }}>
