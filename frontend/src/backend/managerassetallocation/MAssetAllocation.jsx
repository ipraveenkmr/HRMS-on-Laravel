...
export default function Attendance() {
  ...
  const assets = usecdotStore((state) => state.assets).sort((a, b) =>
    a.id > b.id ? -1 : 1
  );
  ...
  const employeeApi = async () => {
    // starting
    await axios
      .get(baseURL + "empbydept/" + emp_department + "/")
  ...
  const assetApi = async () => {
    // starting
    await axios
      .get(baseURL + "assetbydept/" + emp_department + "/")
      .then(function (response) {
  ...
  const handleChangeDepartment = async (event) => {
    ...
    if (event.target.value) {
      await axios
        .get(baseURL + "assetbydept/" + event.target.value + "/")
        .then(function (response) {
